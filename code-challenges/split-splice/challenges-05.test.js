...
const templateWithJQuery = () => {
  starWarsPeople.forEach((person) => {
    ...
    const $newPerson = $(`<section>${template}</section>`);
    //find template items and populate them
    $newPerson.find("h2").text(person.name);
    $newPerson.find("h3").text(person.height);
    $newPerson.find("p").text(person.eye_color);
    //append to the dom
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((ingredient) => {
    var index = ingredient.indexOf(" ", ingredient.indexOf(" ") + 1);
    var item = ingredient.substr(index + 1);
    result.push(item);
  });
  return result;
};
...
//   let result = [];
//   recipe.ingredients.forEach((ingredient) => {
//     let foodArray = ingredient.split(" ");
//     let recipeIngredients = "";
//     for (let i = 2; i < foodArray.length; i++) {
//       recipeIngredients += foodArray[i];
//       if (i !== foodArray.length - 1) {
//         recipeIngredients += " ";
//       }
//     }
//     result.push(recipeIngredients);
//   });
//   return result;
// };
/* ------------------------------------------------------------------------------------------------
